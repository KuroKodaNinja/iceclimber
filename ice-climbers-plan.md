...
| 60 | **Console auto-shows the agent's stream — no `--agent-log` on the Popo side.** The `[NANA]` pane was fed only by a controller-local `--agent-log` file (the demo tee'd `limactl shell … | tee host-file`), so an operator who just ran the agent saw an empty pane while 22 requests scrolled by in `[POPO]`. Now: run headless (no tty), the `nana` launcher mirrors the agent's stdout+stderr to `$ROOT/agent/<name>/session.log` (`[ -t 1 ]`-gated, so interactive sessions keep their tty and aren't broken by a pipe); and the console — which already holds the sandbox's SSH/SFTP session — **auto-discovers and tails those per-agent logs over the transport** (`pollAgentLogs` reads each `agent/*/session.log`, offset-tracked, emitting new lines as `tui.AgentLine`; name-prefixed when several agents). No flag. The empty-pane hint is now actionable ("run `$ROOT/nana` in the sandbox"). `--agent-log` stays as an explicit override; the passive `tui`/`logs` attach views (no live SSH session) remain file-based. Dogfooded: a functional test asserts a headless `nana` run writes `session.log`; a cli unit test drives `pollAgentLogs` over a real ExecFS (single/incremental/truncation/multi-agent); a teatest asserts the hint + a streamed `AgentLine` render in `[NANA]` | The console promises a single pane of glass, but the agent runs *in the sandbox* and its stream lived nowhere the controller could see without manual tee'ing — so the common case (run agent, watch console) silently showed nothing. Capturing at the launcher (where the agent actually runs) + tailing over the session the console already has makes it work with zero operator setup. tty-gating is the crux: tee'ing always would break interactive Claude's UI, so capture only when headless — which is exactly the "watch from the console" case. Architected per-agent (one stream each) so multi-agent — and later multi-sandbox — fan-out slots in |
