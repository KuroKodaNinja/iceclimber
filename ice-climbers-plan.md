...
**Post-v1 increments (shipped):** Tier 0→1 pip auto-fallback (#25); **Node/npm**, the second language (#44); the **observability TUI** (#45); and the **TUI-first operator console** — bare `iceclimber`, serve embedded with inline modal approvals (#46, Phase 1).

**Console roadmap (incremental, #46):** **Phase 2** — management actions in the TUI (trigger installs python/node/pip/npm from a form; an egress **rules + pending** view with approve/deny/forget; **bootstrap/re-provision**; a fuller status panel polling queue/runtimes/capabilities). **Phase 3** — **multi-sandbox** (a sandbox list/switcher; serve several at once — model state is already keyed per `sandbox_id`).
