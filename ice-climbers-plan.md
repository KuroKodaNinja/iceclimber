...
Status: **v1 + post-v1 increments complete and verified end-to-end** against a real Alpine/musl sandbox (all §12 phases ✅, decisions #44–#54). Shipped on top of v1: **three languages** — Python/pip, JavaScript/npm, Java/Maven, each with runtime + packages over Tier 0 (mirror) and Tier 1 (relay) — and the **TUI-first operator console** (Phases 1+2). The pre-fleet hardening pass (#54) closed the console error-visibility gaps and the `sandbox_id` namespacing invariant is now tested. The `ExecFS` bulk-transfer (decision #55) shipped too — a runtime tree now pushes in one `tar` exec, so all three languages install over either transport (no SFTP-only constraint). Remaining work: **multi-sandbox** (the fleet — Phase 3) and the v2 backlog (sub-agent/`web.research`, Tier 2 build). No open v1 polish items.
