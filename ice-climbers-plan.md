...
**Console roadmap (incremental, #46–#47):** **Phase 2a (done, #47)** — install form + bootstrap confirm in the TUI. **Phase 2b (next)** — the remaining management actions: an egress **rules + pending** view (approve/deny/forget), and a fuller **status panel** polling heartbeat/queue/runtimes/capabilities. **Phase 3** — **multi-sandbox** (a sandbox list/switcher; serve several at once — model state is already keyed per `sandbox_id`).
