...
| 47 | **Operator-initiated installs + bootstrap from the console** (Phase 2a of #46). `i` opens an install form, `b` a bootstrap confirm; the operator drives the sandbox from within the TUI, not only observes/approves. Built on an injected **`OpRunner`** seam (mirrors the asker seam): the `tui` owns the form UI (**charmbracelet/huh**) + a neutral `InstallRequest`; the cli's `consoleOps` holds the session and executes off the UI goroutine, emitting a `KindOperated` activity event (so the `[POPO]` pane + JSONL reflect it like a serviced request) + an `OpResultMsg` to clear the running indicator. The running state single-flights operator actions; the approval modal still preempts. Bootstrap reuses an extracted `provision()` (tree + pip.conf + NANA.md + ping/pong smoke test); destructive `--force` stays CLI-only. huh hides at the group level, so pip/npm's package+tier fields live in a second group skipped for python/node runtimes | The console promised "manage the sandbox from within"; observe+approve (#46) was only half. Pre-provisioning without waiting for the agent to ask is the natural next capability, and the `OpRunner` seam keeps `tui` free of session/install knowledge just as the asker seam kept it free of approval-routing. First form dependency (huh), justified by Phase 2's growing form surface (egress rules next) |
