...
limactl shell "$DEMO" -- mkdir -p "$ICECLIMBER_HOME/work"
sed "s#{{ICECLIMBER_HOME}}#$ICECLIMBER_HOME#g" "$HERE/../demo/TASK.md" \
	| limactl shell "$DEMO" -- sh -c "cat > '$ICECLIMBER_HOME/work/TASK.md'"

nana="$ICECLIMBER_HOME/nana"
...
# TASK.md uses absolute {{ICECLIMBER_HOME}}/work paths, so no cwd dependency. The nana launcher
...
prompt="Read $ICECLIMBER_HOME/work/TASK.md and complete the task it describes, exactly."
