...
**Egress modes** (decisions #76/#77/#78): the default is now **`egress_mode: proxy`** — the
sandbox's own package managers reach real registries through a controller-run MITM proxy
over the `ssh -R` tunnel (host + package/path-level policy; `maven build` builds online
there, and native-tool egress needs an active `serve`). **`egress_mode: relay`** (controller
fetches, Popo relays artifacts in, sandbox opens no connection at all) is the opt-in stricter
air-gap for compliance regimes that want only relayed files. The `ExecFS`
